...
function rpc(request, callback) {
  ...
  var errorMessage = 'Zcash JSON-RPC: ';
  ...
  var req = this.protocol.request(options, function(res) {
    ...
    res.on('end', function() {
      ...
      if (res.statusCode === 500 && buf.toString('utf8') === 'Work queue depth exceeded') {
        var exceededError = new Error('Zcash JSON-RPC: ' + buf.toString('utf8'));
...
RpcClient.callspec = {
  ...
  z_listReceivedByAddress: 'int bool',
